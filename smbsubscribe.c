//
// subscriber.c
// Call: ./smbsubscribe.c brokerIP topic
// -> to subscribe all topics call ./smbsubscribe brokerIP '#'
...
int main(int argc, char **argv)
{
  ...
  if (argc != 3)
  {
    fprintf(stderr, "call: %s brokerIP topic\n", argv[0]);
    return 1;
  }
  // 1. Element -> server ip
  server_ip = argv[1];
  ...
  fprintf(stderr, "broker ip: %s\ntopic: %s\n", server_ip, topic);

  strcpy(broker_message, "S");
  ...
  while (1)
  {
    fprintf(stderr, "waiting for incoming messages...\n");
